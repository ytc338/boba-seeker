...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Calculate mismatches but do not commit changes",
    )
    ...
    try:
        ...
        occupied_ids = set(
            dev_session.query(Shop.id).all()
        )  # List of tuples, really separate needed
        ...
        dev_session.execute(
            text("SELECT setval('shops_id_seq', (SELECT MAX(id) FROM shops))")
        )
