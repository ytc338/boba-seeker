...
def create_brand_and_link(conn, brand_name, shop_ids):
    ...
    conn.execute(
        text("INSERT INTO brands (id, name, created_at, updated_at) VALUES (:id, :name, :now, :now)"),
    ...
    conn.execute(
        text("UPDATE shops SET brand_id = :brand_id, updated_at = :now WHERE id IN :ids"),
...
def interactive_brand_creator(min_size=3):
    ...
    with engine.connect() as conn:
        ...
        shops = [{"id": row.id, "name": row.name, "address": row.address} for row in result]
    ...
    with engine.begin() as conn: # Transaction block
        for i, cluster in enumerate(clusters):
            ...
            if choice == 'q':
                ...
                new_name = input(f"Enter new brand name (default: {cluster['name']}): ").strip()
                final_name = new_name if new_name else cluster['name']
                shop_ids = [m['id'] for m in cluster['members']]
                create_brand_and_link(conn, final_name, shop_ids)
            elif choice == 's':
                print("Enter numbers of shops to INCLUDE (comma separated, e.g. 1,3,5):")
                selection = input("> ").strip()
                try:
                    indices = [int(x.strip()) - 1 for x in selection.split(",")]
                    selected_shops = [cluster['members'][idx] for idx in indices if 0 <= idx < len(cluster['members'])]
                    ...
                    new_name = input(f"Enter brand name for these {len(selected_shops)} shops (default: {cluster['name']}): ").strip()
                    final_name = new_name if new_name else cluster['name']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-size", type=int, default=3, help="Minimum cluster size to show")
