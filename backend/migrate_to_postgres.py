...
def migrate_data(sqlite_path: str, postgres_url: str, batch_size: int = 1000):
    ...
    try:
        ...
        brand_columns = sqlite_session.execute(
            text("PRAGMA table_info(brands)")
        ).fetchall()
        ...
        print("\n📋 Migrating shops...")
        total_shops = sqlite_session.execute(
            text("SELECT COUNT(*) FROM shops")
        ).scalar()
        print(f"   Found {total_shops} shops to migrate")

        shop_columns = sqlite_session.execute(
            text("PRAGMA table_info(shops)")
        ).fetchall()
