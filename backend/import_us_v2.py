...
NATIONAL_BRANDS = [
    ...
    },
    {
        "name": "TP Tea",
        "name_zh": "茶湯會",
        "aliases": ["TP TEA", "Tpumps"],
        "origin_country": "TW",
    },
    {
        "name": "Ten Ren",
        "name_zh": "天仁茗茶",
        "aliases": ["Ten Ren's Tea"],
        "origin_country": "TW",
    },
    {
        "name": "Xing Fu Tang",
...
REGIONAL_BRANDS = {
    ...
    "NorCal": [  # Bay Area
        ...
        {
            "name": "Urban Ritual",
            "name_zh": "Urban Ritual",
            "aliases": [],
            "origin_country": "US",
        },
        {
            "name": "Little Sweet",
            "name_zh": "Little Sweet",
            "aliases": [],
            "origin_country": "US",
        },
        {
    ...
    "Hawaii": [  # Honolulu
        ...
        {
            "name": "Thang's French Coffee & Bubble Tea",
            "name_zh": "Thang's",
            "aliases": [],
            "origin_country": "US",
        },
...
async def discovery_search(
    ...
    for point in grid:
        ...
        for shop_data in shops_data:
            ...
            matched_brand = (
                next((b for b in db_brands if b.name == matched_data["name"]), None)
                if matched_data
                else None
            )
    ...
    db.commit()
    logger.info(
        f"Discovery result for {city}: {discovered} total, {linked} linked to brands"
    )
    return discovered


async def import_city(
    service, db, city: str, session_place_ids: set, discover: bool = True
) -> int:
...
def sync_to_database(target_url: str):
    ...
    try:
        ...
        existing_target_ids = {
            id[0]
            for id in target_db.query(Shop.google_place_id)
...
async def main():
    parser = argparse.ArgumentParser(
        description="US Boba Import v2 - Better Brand Coverage"
    )
    parser.add_argument("--city", type=str, help="Single city to import")
    parser.add_argument(
        "--discover-only", action="store_true", help="Only run discovery mode"
    )
    parser.add_argument(
        "--no-discover", action="store_true", help="Skip discovery mode"
    )
    ...
    try:
        ...
        for city in cities:
            if args.discover_only:
                ...
                count = await import_city(
                    service, db, city, session_place_ids, discover=discover
                )
            grand_total += count
